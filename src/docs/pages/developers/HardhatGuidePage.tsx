...
const HardhatGuidePage: React.FC = () => {
  return (
    <div>
      <Heading level={1}>Hardhat Guide</Heading>
      
      <Paragraph>
        Hardhat is a development environment for Ethereum software that makes it easy to compile, 
        deploy, test, and debug your smart contracts. This guide shows how to use Hardhat with Ramestta.
      </Paragraph>
...
# Choose "Create a JavaScript project"`}
      ...
      <Heading level={2}>Configure for Ramestta</Heading>
  ...
  etherscan: {
    ...
    customChains: [
      ...
      <CodeBlock
        title=".env"
        language="bash"
        code={`PRIVATE_KEY=your_wallet_private_key_here
RAMASCAN_API_KEY=your_ramascan_api_key`}
      />

      <InfoBox type="warning" title="Security">
        Never commit your <code>.env</code> file. Add it to <code>.gitignore</code>.
      </InfoBox>

      <Divider />

      <Heading level={2}>Write a Smart Contract</Heading>

      <CodeBlock
        title="contracts/HelloRamestta.sol"
        language="solidity"
...
contract HelloRamestta {
    ...
    function getInfo() external view returns (
      ...
      <Heading level={2}>Compile Contract</Heading>
...
# Compiled 1 Solidity file successfully`}
      ...
      <Heading level={2}>Write Deployment Script</Heading>
...
main()
  ...
  .catch((error) => {
    ...
    process.exit(1);
      ...
      <Heading level={2}>Deploy to Ramestta</Heading>

      <CodeBlock
        title="Deploy Commands"
        language="bash"
        code={`# Deploy to Ramestta Testnet first
...
npx hardhat run scripts/deploy.js --network ramestta`}
      ...
      <InfoBox type="info" title="Gas Costs">
        Ramestta has very low gas costs. A typical contract deployment costs less than $0.10 worth of RAMA.
      </InfoBox>

      <Divider />

      <Heading level={2}>Verify Contract</Heading>

      <CodeBlock
        title="Verify on RamaScan"
        language="bash"
        code={`# Verify with constructor arguments
npx hardhat verify --network ramestta <CONTRACT_ADDRESS> "Hello from Ramestta!"

# Example:
npx hardhat verify --network ramestta 0x1234...5678 "Hello from Ramestta!"`}
      />

      <Heading level={2}>Write Tests</Heading>

      <CodeBlock
        title="test/HelloRamestta.test.js"
        language="javascript"
...
npx hardhat test test/HelloRamestta.test.js`}
      />

      <Heading level={2}>Interact with Deployed Contract</Heading>

      <CodeBlock
        title="scripts/interact.js"
        language="javascript"
        code={`const hre = require("hardhat");
...
main().catch(console.error);`}
      ...
      <Heading level={2}>Hardhat Tasks</Heading>

      <CodeBlock
        title="Custom Hardhat Task"
        language="javascript"
        code={`// Add to hardhat.config.js
...
// npx hardhat block --network ramestta`}
      />

      <Heading level={2}>Useful Commands</Heading>
      ...
      <Table
        ...
        rows={[
          ...
          ['npx hardhat run scripts/deploy.js --network ramestta', 'Deploy to mainnet'],
          ['npx hardhat verify --network ramestta <address> <args>', 'Verify contract'],
          ['npx hardhat console --network ramestta', 'Interactive console'],
      ...
      <InfoBox type="success" title="Next Steps">
        Your contract is deployed! Check it on <a href="https://ramascan.com" target="_blank" rel="noopener noreferrer" className="text-primary-400 hover:underline">RamaScan</a>, 
        or learn about <a href="/docs?page=sdk-overview" className="text-primary-400 hover:underline">integrating with the SDK</a>.
      </InfoBox>
    </div>
  );
};

export default HardhatGuidePage;
